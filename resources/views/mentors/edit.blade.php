            ...
            @foreach ($student->mentors as $index => $mentor)
                <div class="input-group mb-2 mentor-select">
                    <select name="mentors[]" class="form-select">
                        ...
                        @foreach ($mentors as $availableMentor)
                            <option value="{{ $availableMentor->id }}" {{ $availableMentor->id == $mentor->id ? 'selected' : '' }}>
                                {{ $availableMentor->first_name }} {{ $availableMentor->last_name }}
                            </option>
                        @endforeach
                    </select>
